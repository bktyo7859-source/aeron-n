import { ProductsDB, UsersDB } from './config/db.js';
...
const INITIAL_PRODUCTS = [
  ...
  },
  {
    ...
    isFeatured: true,
    isNewRelease: true,
    description: "Designed for competitive runners seeking maximum energy efficiency.",
    defaultColor: "white",
  ...
  },
  {
    ...
    isFeatured: true,
    isNewRelease: false,
    description: "Architectural lines meet everyday luxury.",
    defaultColor: "black",
  ...
  },
  {
    ...
    isFeatured: true,
    isNewRelease: true,
    description: "Ultra-lightweight sprint trainer engineered for quick cadence.",
    defaultColor: "silver",
  ...
  },
  {
    ...
    isFeatured: false,
    isNewRelease: true,
    description: "Pure functional training stability.",
    defaultColor: "black",
  ...
  },
  {
    ...
    isFeatured: true,
    isNewRelease: true,
    description: "High-top tactical lockdown with targeted ankle collar support.",
    defaultColor: "red",
...
export const seedDatabase = async () => {
  console.log("Seeding AERON database...");

  // Seed Products if empty
  const existingProducts = ProductsDB.find();
  if (existingProducts.length === 0) {
    INITIAL_PRODUCTS.forEach((p) => ProductsDB.insert(p));
    console.log(`Seeded ${INITIAL_PRODUCTS.length} AERON products.`);
  }

  // Seed Default Admin User if empty
  const adminEmail = "admin@aeron.lab";
  const existingAdmin = UsersDB.findOne((u) => u.email === adminEmail);
  if (!existingAdmin) {
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash("AeronAdmin2026!", salt);
    UsersDB.insert({
      name: "AERON Admin",
      email: adminEmail,
    ...
    console.log("Default Admin user created: admin@aeron.lab / AeronAdmin2026!");
  }
};
